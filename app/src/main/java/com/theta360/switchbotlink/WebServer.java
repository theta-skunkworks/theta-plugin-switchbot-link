...
public class WebServer extends NanoHTTPD {
    ...
    private Response servePreviewCommnand(Method method, String uri, String strJson) {
        ...
        try {
            ...
            if ( name.equals("camera.getPreviewFrame") ) {
                ...
                String result =  mCallback.getSwitchBotList();
                return newFixedLengthResponse(Response.Status.OK, "text/plain", "{\"name\":\"camera.getSwitchBotList\", \"results\":" + result + "}");

            } else if ( name.equals("camera.setTrigger") ) {
                String setMac="";
                if ( !command.isNull("parameters") ) {
                    JSONObject parameters = command.getJSONObject("parameters");
                    if ( !parameters.isNull("mac") ) {
                        setMac = parameters.getString("mac");
                    }
                }
                Log.d(TAG,"setTriggerMac" + setMac);
                mCallback.setTriggerMac(setMac);
                return newFixedLengthResponse(Response.Status.OK, "text/plain", "{\"name\":\"camera.setTrigger\", \"state\":\"done\"}");
                ...
                if ( !command.isNull("parameters") ) {
                    JSONObject parameters = command.getJSONObject("parameters");
                    if ( !parameters.isNull("mac") ) {
                        getMac = parameters.getString("mac");
                    }
                }
                Log.d(TAG,"getHumidityList:MAC=" + getMac);
                String result = mCallback.getHumidityList(getMac);
                return newFixedLengthResponse(Response.Status.OK, "text/plain", "{\"name\":\"camera.getHumidityList\", \"results\":" + result + "}");
    ...
    public interface Callback {
        ...
        byte[] getLatestFrame();
        String getSwitchBotList();
        void setTriggerMac(String setMac);
        String getHumidityList(String setMac);
    }
